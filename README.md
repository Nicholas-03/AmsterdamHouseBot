...
The Cloudflare mailbox Worker lives in `cloudflare-mailbox-worker/`. It expects a KV namespace bound as `MAILBOX`, a secret named `API_TOKEN`, and an Email Routing rule that sends the chosen inbox address to the Worker. The HTTP API exposes `/health`, authenticated `/messages`, `/messages/{id}`, `/messages/{id}/seen`, and manual `POST /messages/rebuild-index`. Use `cloudflare-mailbox-worker/wrangler.toml.example` as the local template, but keep the real token and namespace IDs out of git. Set the Worker plain-text variable `FORWARD_TO` to a verified Cloudflare Email Routing destination if you also want every new mailbox email forwarded after it is saved for the bot. New messages include a `forward.status` value in the mailbox API so failed forwarding is visible. `/messages` reads a maintained `message:index` KV key instead of calling `KV.list()`, because the Cloudflare free tier has a low daily list-operation quota. Use `POST /messages/rebuild-index` only manually after the quota resets if older pre-index messages need to be recovered.

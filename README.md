...
- `ROOFZ_EMAIL`: optional, defaults to `KAMERNET_EMAIL`
...
- `ROOFZ_PREAPPLICATION_ENABLED`: optional, set to `1` to poll Gmail for unread Roofz pre-application links after the first contact request
- `ROOFZ_GMAIL_CREDENTIALS_PATH`: optional, defaults to `gmail_credentials.json`; OAuth desktop client JSON for the official Gmail API
- `ROOFZ_GMAIL_TOKEN_PATH`: optional, defaults to `gmail_token.json`; saved Gmail OAuth token generated by `scripts/gmail_authorize.py`
...
To allow a live Roofz send, set `ROOFZ_REPLY_DRY_RUN=0` and pass `--live` for one listing. Roofz pre-applications use the official Gmail API with read-only scope. Save an OAuth desktop client as `gmail_credentials.json`, then run:

```bash
python scripts/gmail_authorize.py
python scripts/check_roofz_preapplications.py --title "Jan van Galenstraat 502"
```
